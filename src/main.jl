...
add_arg_table!(CLI_SETTINGS,
    ...
    Dict(
        :arg_type => String,
        :help => "method used for choosing the Tikhonov regularization parameter. One of \"gcv\", \"lcurve\", \"reginska\", \"chi2\", \"mdp\", or \"none\". These flags correspond to generalized cross-validation, the L-curve method, Reginska's minimum-product criterion, the chi-squared method, Morozov's discrepency principle, and zero regularization. Required parameter",
        :group => :t2_map_part_required,
    ),
    "--RegParams",
    Dict(
...
add_arg_table!(CLI_SETTINGS,
    ...
    Dict(
        :arg_type => Int,
        :help => "in estimating the local refocusing flip angle to correct for B1 inhomogeneities, up to --nRefAngles angles in the range [--MinRefAngle, 180] are explicitly checked. The optimal angle is then estimated by interpolating between these observations. (default: 64)",
        :group => :B1_SE_corr,
    ),
    "--nRefAnglesMin",
    Dict(
        :arg_type => Int,
        :help => "initial number of angles in the range [--MinRefAngle, 180] to check during flip angle estimation before refinement near likely optima. Setting --nRefAnglesMin equal to --nRefAngles forces all candidate angles to be checked before interpolation. (default: 9)",
        :group => :B1_SE_corr,
    ),
    "--MinRefAngle",
    Dict(
